...
@section('title', __('My Repository') . ' | Net-Library Antigravity')

@section('content')
    <main class="max-w-7xl mx-auto px-10 py-10">
        <header class="mb-20">
            <h2 class="text-5xl font-black tracking-tighter mb-4 text-dark-navy dark:text-white transition-colors duration-300">{{ __('My Assets') }}</h2>
            <p class="text-gray-400 dark:text-white/30 font-light text-lg">{{ __('Detailed overview of your current loans and historical engagement.') }}</p>
        </header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
            <div class="bg-white/40 dark:bg-white/5 backdrop-blur-md border border-sky-blue/10 dark:border-white/5 rounded-[2.5rem] p-10">
                <p class="text-gray-400 dark:text-white/20 text-[10px] font-black uppercase tracking-[0.3em] mb-4">{{ __('In Possession') }}</p>
                <h3 class="text-5xl font-black text-sky-blue">{{ $stats['borrowed'] }} <span class="text-sm font-light text-gray-400 dark:text-white/20 ml-2 tracking-widest uppercase">{{ __('Books') }}</span></h3>
            </div>
            <div class="bg-white/40 dark:bg-white/5 backdrop-blur-md border border-sky-blue/10 dark:border-white/5 rounded-[2.5rem] p-10">
                <p class="text-gray-400 dark:text-white/20 text-[10px] font-black uppercase tracking-[0.3em] mb-4">{{ __('Account Fines') }}</p>
                <h3 class="text-4xl font-black text-red-500">Rp{{ number_format($stats['total_fines'], 0, ',', '.') }}</h3>
            </div>
            <div class="bg-white/40 dark:bg-white/5 backdrop-blur-md border border-sky-blue/10 dark:border-white/5 rounded-[2.5rem] p-10">
                <p class="text-gray-400 dark:text-white/20 text-[10px] font-black uppercase tracking-[0.3em] mb-4">{{ __('Engagement History') }}</p>
                <h3 class="text-5xl font-black text-emerald-500 dark:text-emerald-400">{{ $stats['history'] }} <span class="text-sm font-light text-gray-400 dark:text-white/20 ml-2 tracking-widest uppercase">{{ __('Records') }}</span></h3>
            </div>
        </div>
            ...
            <template x-if="reviewModal">
                <div class="fixed inset-0 z-[100] flex items-center justify-center p-10">
                    ...
                    <div x-data="{ hoverRating: 0, selectedRating: 0 }" class="relative w-full max-w-xl bg-white dark:bg-dark-navy border border-sky-blue/20 rounded-[3rem] p-12 shadow-2xl animate-in zoom-in-95 duration-300">
                        <h3 class="text-3xl font-black text-dark-navy dark:text-white mb-2 tracking-tighter">{{ __('Share Experience') }}</h3>
                        ...
                        <form :action="'{{ url('/books') }}/' + selectedBook.id + '/review'" method="POST" class="space-y-10">
                            @csrf
                            <div>
                                <label class="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 dark:text-white/20 mb-6 block text-center">{{ __('Efficiency Rating') }}</label>
                            ...
                            <div>
                                <label class="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 dark:text-white/20 mb-4 block">{{ __('Detailed Log') }}</label>
                                <textarea name="review" rows="4" class="w-full bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/5 rounded-2xl p-6 text-dark-navy dark:text-white focus:border-sky-blue focus:ring-0 transition-all font-medium placeholder:text-gray-300 dark:placeholder:text-white/5" placeholder="{{ __('Share your insights about this asset...') }}"></textarea>
                            </div>

                            <div class="flex gap-4">
                                <button type="button" @click="reviewModal = false" class="flex-grow py-5 border border-gray-100 dark:border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 transition-all">{{ __('Cancel') }}</button>
                                <button type="submit" class="flex-grow py-5 bg-sky-blue text-dark-navy rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] shadow-glow hover:scale-105 transition-all">{{ __('Transmit Review') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
